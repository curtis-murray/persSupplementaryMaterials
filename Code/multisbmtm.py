...
#!/usr/bin/python3
'''
Description: 2-layer hierarchical SBM based on doc-word network and doc-metadata network.
Original Author: Chris Hyland and Yuanming Tao from https://github.com/martingerlach/hSBM_Topicmodel
Adapted for use in probabilistic emotion recommender system by Curtis Murray
'''
...
class sbmmultilayer:
    ...
    def make_graph(self, list_texts, list_titles, list_metadata):
        """
        ...
        list_metadata : type
            Description of parameter `list_metadata`.
        Returns
        -------
        type
        ...
        # Documents - 'title', words - 'word', metadata_items - 'metadata_item'
        name = g.vp["name"] = g.new_vp("string")
        # Documents nodes (0), word nodes (1), metadata nodes (2)
        ...
        # metadata-doc edge (1) and doc-word edge (0)
        ...
        metadata_verticies = defaultdict(lambda: g.add_vertex())
        ...
        #### Construct bipartite metadata-doc graph ####
        # Create edges between metadata items and docs
        for doc_id in range(D):
            title = list_titles[doc_id]
            text = list_texts[doc_id]
            d = doc_vertices[title]
            ...
            metadata_items = list_metadata[doc_id]
        
            for metadata_item in metadata_items:
                m = metadata_verticies[metadata_item]
                name[m] = metadata_item
                kind[m] = 2 # metadata node
                vlayers[m] = [1]
                e = g.add_edge(d, m) # add link between document and word node
                edgeCount[e] = 1 # assign weighting to edge based on number of occurrences
                edgeType[e] = 1 # to indicate the edge is metadata-document edge
        ...
        self.meta = [ g.vp['name'][v] for v in  g.vertices() if g.vp['kind'][v]==2   ]
    ...
    def fit(self):
        ...
        seed_rng(self.random_seed)
        mdl = np.inf
        # Fit n_init random initializations to avoid local optimum of MDL.
        for _ in range(self.n_init):
            # Enables the use of LayeredBlockState. Use a degree-core
            # rected layered SBM.
    ...
    def get_groupStats(self, l=0):
        ...
        M = self.get_M() # number of meta nodes
        ...
        n_mb = np.zeros((M,B))  # will be reduced to (M, B_m)

        n_dbm = np.zeros((D,B))   # will be reduced to (D, B_m)
        ...
        for e in g.edges():
            ...
            if g.ep.edgeType[e] == 1:
                # z1 will have values from 1, 2, ..., B_d; document-group i.e document block that doc node is in
                # z2 will have values from B_d + B_w, ..., B_d + B_m - 1; metadata-group i.e metadata block that metadata type node is in
                z1, z3 = state_l_edges[e]
                # v1 ranges from 0, 1, 2, ..., D - 1
                # v2 ranges from D+V, ..., D+V+M-1 (V # of meta types)
                v1 = int(e.source()) # document node index
                v3 = int(e.target()) # meta type node index
                n_mb[v3-(D+V),z3] += 1 # meta type v3 is in topic z3
                n_dbm[v1,z3] += 1 # document v1 has a meta tag in topic z3
            

        # Retrieve the corresponding submatrices
        n_db = n_db[:, np.any(n_db, axis=0)] # (D, B_d)
        n_wb = n_wb[:, np.any(n_wb, axis=0)] # (V, B_w)
        n_dbw = n_dbw[:, np.any(n_dbw, axis=0)] # (D, B_w)
        n_mb = n_mb[:, np.any(n_mb, axis=0)] # (M, B_m)
        n_dbm = n_dbm[:, np.any(n_dbm, axis=0)] # (D, B_m)]

        B_d = n_db.shape[1]  # number of document groups
        B_w = n_wb.shape[1] # number of word groups (topics)
        B_m = n_wb.shape[1] # number of meta groups (topics)
        ...
        # Probability of topic-meta over meta type
        p_tm_m = (n_mb / np.sum(n_mb, axis=1)[:, np.newaxis]).T

        # Mixture of meta-groups into documents P(t_m | d), shape B_m x D
        p_tm_d = (n_dbm / np.sum(n_dbm, axis=1)[:, np.newaxis]).T

        # Per-meta-topic meta distribution, shape M x B_m
        p_m_tm = n_mb / np.sum(n_mb, axis=0)[np.newaxis, :]

        #p_m_w =  p_m_tm * p_tm_d * p_d_td * p_td_w * p_tw_w

        result = {}
        result['Bd'] = B_d # Number of document groups
        result['Bw'] = B_w # Number of word groups
        result['Bm'] = B_m # Number of meta groups
        result['p_tw_w'] = p_tw_w # Group membership of word nodes
        result['p_td_d'] = p_td_d # Group membership of document nodes
        result['p_tm_m'] = p_tm_m # Group membership of word nodes
        result['p_tw_d'] = p_tw_d # Topic proportions over documents
        result['p_w_tw'] = p_w_tw # Topic distribution over words
        result['p_m_tm'] = p_m_tm # Meta-topic distribution over meta
        result['p_tm_d'] = p_tm_d # Meta-topic proportions over documents
...
# Helper methods for nodes
    ...
    def get_M(self):
        '''Return number of meta-nodes == types'''
        return int(np.sum(self.g.vp['kind'].a==2))
